...
interface CharacterLibraryModalProps {
  open: boolean
  onClose: () => void
  onImport: (character: CharacterReference) => void
  excludeNames: string[]
}
...
export function CharacterLibraryModal({
  ...
  const available = characters.filter(
    c => !excludeNames.includes(c.characterName)
  )
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="relative mx-4 max-h-[80vh] w-full max-w-lg overflow-hidden rounded-xl bg-white shadow-xl">
        <div className="flex items-center justify-between border-b px-4 py-3">
          <h3 className="font-semibold text-gray-900">Character Library</h3>
          <button
            type="button"
            onClick={onClose}
          ...
          {!loading && available.length === 0 && (
            <p className="py-8 text-center text-sm text-gray-500">
              {characters.length === 0
                ...
                : 'All library characters are already in this story.'}
            </p>
          )}
